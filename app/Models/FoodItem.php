...
class FoodItem extends BaseModel
{
    ...
    {
        $sql = 'INSERT INTO food_items (vendor_id,category_id,name,description,storage_instructions,price,discount_percent,expiry_date,stock,image_path,status,created_at,updated_at) VALUES (:vendor_id,:category_id,:name,:description,:storage_instructions,:price,:discount_percent,:expiry_date,:stock,:image_path,:status,NOW(),NOW())';
    ...
    public function find(int $id): ?array
    {
        $stmt = $this->db->prepare('
            SELECT fi.*, c.name as category_name, v.business_name as vendor_name, v.location as vendor_location 
            FROM food_items fi 
    ...
    {
        $w = ['fi.status = "active"', 'fi.stock > 0', 'fi.expiry_date > CURDATE()'];
        $p = [];
        
        if (!empty($filters['category_id'])) { 
            $w[] = 'fi.category_id = :category_id'; 
            $p['category_id'] = $filters['category_id']; 
        }
        if (!empty($filters['q'])) { 
            $w[] = '(fi.name LIKE :q OR fi.description LIKE :q OR v.business_name LIKE :q)'; 
            $p['q'] = '%' . $filters['q'] . '%'; 
        }
        ...
        switch ($filters['sort'] ?? 'newest') {
            case 'price_low':
                $orderBy = 'fi.price ASC';
                break;
            case 'price_high':
                $orderBy = 'fi.price DESC';
                break;
            case 'discount':
                $orderBy = 'fi.discount_percent DESC, fi.price ASC';
                break;
            case 'expiry':
                $orderBy = 'fi.expiry_date ASC';
                break;
            default:
                $orderBy = 'fi.created_at DESC';
        }
        
        $sql = 'SELECT fi.*, c.name as category_name, v.business_name as vendor_name, v.location as vendor_location 
                FROM food_items fi 
                LEFT JOIN categories c ON c.id = fi.category_id 
                LEFT JOIN vendors v ON v.id = fi.vendor_id 
                WHERE ' . implode(' AND ', $w) . ' 
    ...
    public function update(int $id, array $data): bool
    {
        $sql = 'UPDATE food_items 
                SET name = :name, description = :description, storage_instructions = :storage_instructions, 
                    price = :price, discount_percent = :discount_percent, expiry_date = :expiry_date,
                    stock = :stock, image_path = :image_path, status = :status,
